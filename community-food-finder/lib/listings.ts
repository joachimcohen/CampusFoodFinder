...
/** "Every week" listings still "in scope" — not past their optional end date. */
export function isEveryWeekInScope(listing: ListingWithRelations, now: Date = new Date()): boolean {
  if (listing.listing_type !== "every_week") return false;
  if (!listing.is_active) return false;
  if (!listing.recurrence_valid_until) return true;
  const today = getLocalParts(now).dateStr;
  return listing.recurrence_valid_until >= today;
}

/** Is this listing visible in the "Happening Now" section right now? */
export function isHappeningNow(listing: ListingWithRelations, now: Date = new Date()): boolean {
  if (!listing.is_active) return false;

  if (listing.listing_type === "special_event") {
  ...
  if (!isEveryWeekInScope(listing, now)) return false;
  if (!listing.recurrence_days || !listing.recurrence_time_start || !listing.recurrence_time_end) {
    return false;
  }

  const { weekday: today, minutesSinceMidnight: nowMinutes } = getLocalParts(now);
...
/** Is this a special-event listing starting within the next 7 days, but not yet started? */
export function isComingUp(listing: ListingWithRelations, now: Date = new Date()): boolean {
  if (!listing.is_active) return false;
  if (listing.listing_type !== "special_event" || !listing.starts_at) return false;
  const start = new Date(listing.starts_at).getTime();
  const diff = start - now.getTime();
  return diff > 0 && diff <= 7 * 24 * 60 * 60 * 1000;
}

/** Minutes until a special-event listing expires, or an every-week listing's current window ends. */
function minutesRemaining(listing: ListingWithRelations, now: Date): number | null {
  ...
  const endMinutes = timeStringToMinutes(listing.recurrence_time_end);
  return endMinutes - getLocalParts(now).minutesSinceMidnight;
}

/** Minutes until a not-yet-started special-event listing begins. */
function minutesUntilStart(listing: ListingWithRelations, now: Date): number | null {
  if (listing.listing_type !== "special_event" || !listing.starts_at) return null;
  return Math.round((new Date(listing.starts_at).getTime() - now.getTime()) / 60000);
}

/** Minutes until an every-week listing's *next* occurrence begins (0 if active now). */
...
/** Sort key for the "Every Week" section — soonest-upcoming first. */
export function everyWeekSortKey(listing: ListingWithRelations, now: Date): number {
...
export function getTimeRemainingLabel(listing: ListingWithRelations, now: Date = new Date()): string {
  ...
  if (listing.listing_type === "special_event" && listing.expires_at) {
    const end = new Date(listing.expires_at);
    const nowParts = getLocalParts(now);
    const endParts = getLocalParts(end);
    const tomorrowParts = getLocalParts(new Date(now.getTime() + 24 * 60 * 60 * 1000));
...
/** "Wed 7:30–9:30am" style label for the Every Week card. */
...
/** Great-circle distance in kilometres — used for the suburb+distance sort (Section 7.1). */
